...
def xyz2rthph(x,y,z,drho=0.0,dtheta=0.0,dphi=0.0):
    """
        input: x,y,z 
        optional: Rotation
            drho  : displace rho    
            dtheta: rotate   theta [radians]
            dphi  : rotate   phi   [radians]            
        output: 
            rho  : radial distance , [0.0,inf ) 
            theta: angle           , [0.0,pi  ]   radians
            phi  : phi             , [0.0,2*pi]   radians 
    ...
    rho   = np.sqrt(xsq+ysq+zsq) + drho
    theta = np.arctan2(ssq,z)    + dtheta
...
def rthph2zdecra(rho,theta,phi,drho=0.0,dtheta=0.0,dphi=0.0,params=[0.3,0.7,-1.0,0.0]):
    """
        input:
            rho  : radial distance , [0.0,inf ) 
            theta: angle           , [0.0,pi  ]   radians
        ...
        output:
            z    : redshift        , [0.0,inf]
            dec  : Declination     , [-90.,90] deg
            ra   : Right Ascesion  , [0.0,360] deg
    """
    #h = params[4]
    redshift = cosmology.get_dist(rho+drho,params=params,z2radial=True)
    dec = 90. - 180./np.pi*(theta+dtheta)
    ra  = 180./np.pi*(phi + dphi)
    return redshift,dec,ra

def xyz2zdecra(x,y,z,drho=0.0,dtheta=0.0,dphi=0.0,params=[0.3,0.7,-1.0,0.0]):
    '''         
        input   :   x,y,z 
        optional:
            drho  : displace rho
        ...
        Comment:need to rotate to theta and phi so that rthph2zdecra go to right angles
    '''
    rho,theta,phi   = xyz2rthph(x,y,z)
    redshift,dec,ra = rthph2zdecra(rho,theta,phi,drho=drho,dtheta=dtheta,dphi=dphi,params=params) 
    return redshift,dec,ra
...
def nr_str(r,dd,rr,w_smallscale_corr=None,factor=1.0):
    nr_est_gal = np.cumsum(dd*factor)/np.cumsum(rr)
    if w_smallscale_corr!=None:
        nr_est_gal[w_smallscale_corr]=0.
    return nr_est_gal

def nr_jc(r,dd,rr,dr,who_xi='ls',w_smallscale_corr=None,corr_lss=False,factor=1.0):
    nr_est_gal = r*0.
    xi_ls = xi(dd,rr,dr,who_xi=who_xi,corr_lss=corr_lss) 
    if w_smallscale_corr!=None:
        xi_ls[w_smallscale_corr]=0.
    
    xi_ls[np.where(np.isnan(xi_ls))]=0.
    
    xi_ls_r2 = xi_ls * r * r * factor
    # Since 2018 I used: Integration = integrate.cumtrapz(xi_ls_r2,x=r,initial=xi_ls_r2[0]) 
    # Correction from forward email of Marie Claude Cousinou of Nico Haumus
    Integration = integrate.cumtrapz(xi_ls_r2,x=r,initial=0.0) + xi_ls_r2[0] 
...
def nr_gal(r,dd,rr,dr,who_Nest='JC',who_xi='ls',correct_smallscales=False,corr_lss=False,rMin=5.0,factor=1.0):
    if correct_smallscales: w_smallscale_corr = np.where(r<rMin) # default r<5.0
    else: w_smallscale_corr=None

    if   who_Nest=='JC': nr_est_gal = nr_jc(r,dd,rr,dr,who_xi=who_xi,w_smallscale_corr=w_smallscale_corr,corr_lss=corr_lss,factor=factor)
    elif who_Nest=='JM': nr_est_gal = nr_jm(r,dd,rr,dr,w_smallscale_corr=w_smallscale_corr)
    elif who_Nest=='str':nr_est_gal = nr_str(r,dd,rr,w_smallscale_corr=w_smallscale_corr,factor=factor)
...
def get_rh_spline(x,y,cov,nbspl=12,nmock_prec=None,xstart=30,xstop=1300,doplot=True,diagonly=False,logspace=True,who_give='d2',ZOOM='False',ZOOM1='False',cholesky=False,doNrTrick=False,
    ...
    if(doplot):
        ...
        if ZOOM1=='True': pass
        elif ZOOM1=='False': plot(newx,y_spl,'b-',lw=2,)
        plt.errorbar(x,y,yerr=yerr,fmt=colordata+'o',label='Best Fit Spline ('+np.str(nbspl)+' nodes): $\chi^2/\mathrm{ndf}=$'+str('%.3f'%spl.chi2)+'/'+np.str(np.size(thex)-nbspl))  #label='Data')
        #errorbar(thex,they,yerr=theyerr,fmt='yo')
        if ZOOM1=='True': pass
        elif ZOOM1=='False': errorbar(rh,threshold,xerr=drh,fmt='ro',label='$R_H$ = '+str('%.1f'%rh)+' $\pm$ '+str('%.1f'%drh)+' $h^{-1}\mathrm{Mpc}$')
...
def NameZ2(minz=0.43,maxz=0.7,nbins=5):
    """
    returns 2 arrays which contain
    the edges of bins of redshift
    in double and string array 
    ...
    for i in np.arange(nbins):
        znames[i]='z_'+str(np.round(zedge[i],3))+'_'+str(np.round(zedge[i+1],3))
...
def corrCoeffd2(covmatd2,r,mu=None,corrfactor=1.,nbspl=9,name='',save=False,Zmin=-1.,Zmax=1.):
    ...
    plt.xlabel('$r_j\ [h^{-1}\ \mathrm{Mpc}]$',fontsize=20)
    if mu==None: 
        plt.yscale('linear')
    else: pass
    print Y,X
...
def read_cute(filename,normalize=True,give_rightedge=False,ReadComments=True,rebinning=0):
    ...
    if rebinning!=0: r,xi0,dd,dr,rr=numMath.rebinning_all(r,xi0,dd,dr,rr,n=rebinning)
    else: pass

    if ReadComments:
        ng,nr=get_ngnr(filename)
        ...
        return r,xi0,dd,rr,dr

def get_ngnr(filename):
    f=open(filename)
    a=f.readline()
    ng=float(a.split("=")[1].split(" ")[0])
    nr=float(a.split("=")[2].split(" ")[0])
